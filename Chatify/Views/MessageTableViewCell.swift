...
class MessageTableViewCell: UITableViewCell {
    ...
    let playPauseButton: UIButton = .init(type: .system)
    ...
    @objc func handleOpenImageMessage(tapGesture: UITapGestureRecognizer) {
        if videoURL != nil {
            if let thumbnailImageView = tapGesture.view as? UIImageView {
                chatVC?.performZoomInTapGestureForVideoMessage(thumbnailImageView, currentCell: self)
            }
        } else {
            if let imageView = tapGesture.view as? UIImageView {
                chatVC?.performZoomInTapGestureForUIImageViewOfImageMessage(imageView, currentCell: self)
            }
        }
    }
